...
export async function GET(request: NextRequest) {
  ...
  const sportsData: { [key: string]: any[] } = {
    ...
    american_football: [
      ...
      { league: "NFL", home: "Lions", away: "Packers", date: getDateString(3), time: "20:15", icon: "🏈" },
    ],
    tennis: [
      ...
      { league: "Davis Cup", home: "Spain", away: "Australia", date: getDateString(4), time: "15:00", icon: "🎾" },
    ],
    cricket: [
      ...
      { league: "Big Bash", home: "Sydney Sixers", away: "Melbourne Stars", date: getDateString(2), time: "08:00", icon: "🏏" },
    ],
  };

  const events = sportsData[sport] || [];
  
  const eventsWithIds = events.map((e, i) => ({ 
    ...e, 
    id: `${sport}-${i}-${Date.now()}` 
  }));
